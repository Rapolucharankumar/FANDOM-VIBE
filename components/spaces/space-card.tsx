  ...
  return (
    <Link href={`/spaces/${space.id}`}>
      <motion.article
        ...
        className="glass-panel overflow-hidden rounded-[30px] cursor-pointer"
      >
        ...
        <div className="p-5">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-white/10 bg-white/7 p-3">
              ...
              <p className="text-lg font-extrabold text-white">{formatCount(space.members)}</p>
              <p className="text-xs text-white/45">members</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/7 p-3">
              <Radio className="mb-2 size-4 text-neon" />
              <div className="flex items-center gap-2">
                <p className="text-lg font-extrabold text-white">{formatCount(space.liveNow)}</p>
                {space.liveNow > 0 && (
